...
MoncofaApp.DragManager = {
    ...
    setupDragAndDrop() {
            ...
            if (draggedItem) {
                ...
                if (isPitchPlayer) {
                    ...
                    if (isOutside) {
                        ...
                        const pName = MoncofaApp.State.getPlayerById(playerId)?.name || 'jugador';
                        MoncofaApp.UI.showConfirm(
                            'Quitar Jugador',
                            `¿Quitar a ${pName} del campo?`,
                            () => {
        ...
        pitch.addEventListener('drop', (e) => {
            ...
            if (id === 'training-ball') {
                ...
                if (targetAvatar && targetAvatar.dataset.playerId !== id) {
                    const idA = id;
                    const idB = targetAvatar.dataset.playerId;

                    const iA = MoncofaApp.State.lineupIds.indexOf(parseInt(idA));
                    const iB = MoncofaApp.State.lineupIds.indexOf(parseInt(idB));
                    const formation = MoncofaApp.State.getCurrentFormationData();

                    const posA = MoncofaApp.State.customPositions[idA] || (iA !== -1 ? formation[iA] : null);
                    const posB = MoncofaApp.State.customPositions[idB] || (iB !== -1 ? formation[iB] : null);

                    if (posA && posB) {
                        MoncofaApp.State.customPositions[idA] = { x: posB.x, y: posB.y };
                        MoncofaApp.State.customPositions[idB] = { x: posA.x, y: posA.y };
                        MoncofaApp.UI.showToast(`Posiciones intercambiadas`, 'success');
                    }
                } else {
                    MoncofaApp.State.customPositions[id] = { x: xPct, y: yPct };
                }
        ...
        pitch.addEventListener('touchend', (e) => {
            ...
            if (touchStartId === 'training-ball') {
                ...
                if (isOutside) {
                    const pName = MoncofaApp.State.getPlayerById(touchStartId)?.name || 'jugador';
                    MoncofaApp.UI.showConfirm(
                        'Quitar Jugador',
                        `¿Quitar a ${pName} del campo?`,
                        () => {
                    ...
                    // Check if dropped on another player avatar
                    const oldPointerEvents = touchDragItem.style.pointerEvents;
                    touchDragItem.style.pointerEvents = 'none';
                    const targetEl = document.elementFromPoint(touch.clientX, touch.clientY);
                    touchDragItem.style.pointerEvents = oldPointerEvents;

                    const targetAvatar = targetEl ? targetEl.closest('.player-avatar') : null;
                    if (targetAvatar && targetAvatar.dataset.playerId !== touchStartId) {
                        const idA = touchStartId;
                        const idB = targetAvatar.dataset.playerId;
    ...
    performSubstitution(playerOutId, playerInId) {
        ...
        MoncofaApp.UI.showConfirm(
            'Confirmar Cambio',
            `¿Realizar cambio: Entra ${playerIn.name} por ${playerOut.name} (Min ${time}')?`,
            () => {
