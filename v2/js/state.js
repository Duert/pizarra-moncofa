...
MoncofaApp.State = {
    ...
    saveRivals() {
        ...
        try {
            ...
            if (e.name === 'QuotaExceededError' && MoncofaApp.UI) {
                MoncofaApp.UI.showToast("Memoria llena local. Borra equipos.", "error");
            }
        }
    },
    ...
    async addRival(name, logo, shortName) {
        ...
        if (this.supabase) {
            ...
            if (error) {
                ...
                if (MoncofaApp.UI) {
                    MoncofaApp.UI.showToast("Error al guardar en la nube. Revisa la tabla 'rival_teams'.", "error");
                }
            }
        }
        return id;
    ...
    addLog(type, text, data) {
        ...
        if (type === 'goal' || type === 'own_goal_rival' || type === 'penalty_scored') {
            // We scored (or rival scored in own net)
            if (isHome) this.score.home++; else this.score.away++;
        }
        if (type === 'own_goal' || type === 'goal_opponent' || type === 'penalty_rival_scored') {
    ...
    removeLog(id) {
        ...
        if (log) {
            ...
            if (log.type === 'goal' || log.type === 'own_goal_rival' || log.type === 'penalty_scored') {
                if (isHome) this.score.home--; else this.score.away--;
            }
            if (log.type === 'own_goal' || log.type === 'goal_opponent' || log.type === 'goal_away' || log.type === 'penalty_rival_scored') {
                if (isHome) this.score.away--; else this.score.home--;
            }
    ...
    saveCloudSession(session) {
        ...
        if (MoncofaApp.UI) {
            MoncofaApp.UI.updateSyncStatus('syncing');
        }
        ...
        this.saveCloudTimeout = setTimeout(async () => {
            ...
            if (MoncofaApp.UI) {
                if (error) {
                    ...
                    MoncofaApp.UI.updateSyncStatus('offline');
                    MoncofaApp.UI.showToast("Error de sincronización con la nube", "error");
                } else {
                    console.log("Cloud Sync saved");
                    MoncofaApp.UI.updateSyncStatus('connected');
                }
            }
        }, 1000); // 1s debounce to allow player drag to pause/finish before saving
    },
    ...
    clearSession() {
        MoncofaApp.UI.showConfirm(
            'Nuevo Partido',
            '¿Estás seguro de que quieres empezar un NUEVO PARTIDO? Se borrará todo el progreso actual.',
            () => {
                // 1. Clear Storage
                ...
                MoncofaApp.UI.showToast("Partido reiniciado.", "success");
            }
        );
    }
};
