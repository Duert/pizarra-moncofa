...
MoncofaApp.SquadManager = {
        ...
        if (sortedSquad.length === 0) {
            ...
            sortedSquad.forEach(p => {
                ...
                <div class="flex items-center gap-3 p-3 rounded-xl border border-slate-200 bg-white hover:shadow-md transition-all group">
                    ...
                    <div class="relative">
                        ${p.photo ? `
                            <img src="${p.photo}" class="w-10 h-10 rounded-full object-cover border-2 border-white ring-1 ring-black/5">
                        ` : `
                            <div class="w-10 h-10 rounded-full flex items-center justify-center font-black text-white text-sm shadow-sm border-2 border-white ring-1 ring-black/5 ${isGK ? 'bg-yellow-500' : 'bg-red-600'}">
                                ${p.number}
                            </div>
                        `}
                    </div>
    ...
    renderPlayerForm(player = null) {
        ...
        const html = `
            ...
            <form onsubmit="event.preventDefault(); MoncofaApp.SquadManager.savePlayer(${isEdit ? player.id : 'null'})" class="space-y-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Foto del Jugador (Opcional)</label>
                    <div class="flex items-center gap-4 bg-slate-50 p-4 rounded-xl border-2 border-dashed border-slate-200">
                        <div id="p-photo-preview" class="w-16 h-16 rounded-full bg-slate-200 flex items-center justify-center overflow-hidden border-2 border-white shadow-sm">
                            ${player?.photo ? `<img src="${player.photo}" class="w-full h-full object-cover">` : `<i data-lucide="user" class="w-8 h-8 text-slate-400"></i>`}
                        </div>
                        <div class="flex-1">
                            <input type="file" id="p-photo-input" accept="image/*" class="hidden" onchange="MoncofaApp.SquadManager.handlePhotoSelect(this)">
                            <button type="button" onclick="document.getElementById('p-photo-input').click()" class="bg-white border border-slate-300 text-slate-700 px-3 py-2 rounded-lg text-xs font-bold hover:bg-slate-50 transition-all flex items-center gap-2 shadow-sm">
                                <i data-lucide="upload" class="w-4 h-4"></i> Elegir Imagen
                            </button>
                            <p class="text-[10px] text-slate-400 mt-1 italic">Recomendado: Cuadrada</p>
                        </div>
                        <input type="hidden" id="p-photo-data" value="${player?.photo || ''}">
                    </div>
                </div>
    ...
    savePlayer(id) {
        ...
        const photo = document.getElementById('p-photo-data').value;
        ...
        if (id) {
            ...
            if (idx !== -1) {
                squad[idx] = { ...squad[idx], number, name, role, photo };
            }
        } else {
            // CREATE
    ...
    deletePlayer(id) {
        MoncofaApp.UI.showConfirm(
            ...
            () => {
                ...
                MoncofaApp.UI.showToast("Jugador eliminado con éxito.", "success");
            },
            () => {
                this.openManager(); // Refresh list / reopen squad manager
            }
        );
    },
